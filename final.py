...
import re
import os
ffmpeg_path = r'D:\app\ffmpeg-master-latest-win64-gpl\bin'
HEADERS = {
...
def merge_audio_video(video_path, audio_path, output_path):
    # 使用FFmpeg合并音频和视频
    ffmpeg_cmd = f'ffmpeg -i "{video_path}" -i "{audio_path}" -c:v copy -c:a aac -strict experimental "{output_path}"'
...
def main():
    ...
    audio_filename = f"{video_info[4]}.mp3"
    video_filename = f"{video_info[4]}.mp4"

    download_file(video_content[0], audio_filename, f"{video_info[4]} - 音频")
    download_file(video_content[1], video_filename, f"{video_info[4]} - 视频")

    output_filename = f"{video_info[4]}_merged.mp4"
